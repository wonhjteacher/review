...
```
Phase 0 — 랜딩페이지 (DESIGN v3에서 개편)
index.html   Hero(사진 자리) · 오늘 여기 어때?(추천 카드 2장) · 지금 인기(순위 리스트) · 푸터.
             추천 두 코너만 비어 있고 home.js가 채운다.
             체험 데모·기능 미리보기·마무리 CTA는 **뺐다** — 구현 안 된 약속을 화면에 두지 않는다
style.css    DESIGN.md의 토큰·스케일·컴포넌트 수치를 그대로 옮긴 것. 세 페이지가 공유한다
             `.toast`도 여기 있다 — index.html과 save.html이 **같은 마크업을** 쓰기 때문이다
             `.photo-slot`(사진 자리 부품)도 여기 있다 — 지금 쓰는 곳은 Hero 하나다
app.js       PLACES 데이터 + pickPlace() + 데모 상태 관리.
             **v3부터 어느 페이지에도 로드되지 않는다** — 데모가 랜딩에서 빠졌기 때문이다.
             파일은 남겨 두었다: 데모를 되살리면 ①·②의 불변 조건이 그대로 다시 적용된다
site-nav.js  상단 내비게이션(헤더) 필의 선택 상태만 맞춘다. 이동은 <a href="#...">가
             브라우저 기본 동작으로 한다 — 이 파일이 없어도 링크는 그대로 동작한다.
             헤더 마크업 자체는 save.html에도 있다(페이지 링크판, is-active 고정) —
             거기서는 data-nav가 없어 이 스크립트가 스스로 물러난다.
             `#for-you` 필의 hidden 토글은 여기서 하지 않는다 — 그 판단은 home.js가
             #for-you 섹션을 여닫는 곳 한 군데다 (㉖과 같은 이유: 창구가 둘이면 경쟁이 생긴다)

Phase 1 — 랜딩페이지의 추천 두 코너
home.js      「지금 인기」(순위 리스트)와 「오늘 여기 어때?」(카드 2장) 렌더 + 담기.
             생김새는 갈렸지만 담기 버튼·안내 문구·데이터 창구를 공유해 여전히 한 파일이다
...
`index.html`·`home.js`·`home.css`의 클래스도 계약 대상이다 — UI-CONTRACT
「랜딩페이지의 추천 두 코너」·「.pick-row」·「.pick-card」·「.pick-save」.
...
### 데모 흐름 (v3부터 랜딩에 없음 — app.js를 되살릴 때만 해당)
...
①·②는 `app.js`의 조건이다 — v3부터 랜딩에 로드되지 않으므로 **데모를 되살릴 때만** 적용된다.
파일을 지우지 않는 한 조건도 지우지 않는다.
...
**체험 데모의 520px 고정 예외는 v3에서 데모와 함께 빠졌다.** 데모를 되살리면 이 예외도 함께 돌아온다 — 「한 화면에 하나씩」이 그 섹션의 존재 이유였다.
